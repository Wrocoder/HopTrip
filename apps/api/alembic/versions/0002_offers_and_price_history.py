...
def upgrade() -> None:
        ...
        [
            {
                ...
                "notes": "Cached flight data; token required.",
            },
            {
                "code": "amadeus",
                "name": "Amadeus Self-Service",
                "notes": "Technical data candidate; credentials and coverage must be verified.",
            },
        ],
    )
    op.create_table(
    ...
    op.create_index(
        "ix_travel_offers_search",
        "travel_offers",
        ["origin_airport_id", "destination_id", "depart_at"],
    )
    op.create_index(
        "ix_travel_offers_freshness", "travel_offers", ["expires_at", "last_verified_at"]
    )
    ...
    op.create_index(
        "ix_price_observations_data_provider_id", "price_observations", ["data_provider_id"]
    )
    op.create_index(
        "ix_price_observations_origin_airport_id", "price_observations", ["origin_airport_id"]
    )
    op.create_index(
        "ix_price_observations_destination_id", "price_observations", ["destination_id"]
    )
    op.create_index(
        "ix_price_observations_departure_date", "price_observations", ["departure_date"]
    )
    op.create_index("ix_price_observations_observed_at", "price_observations", ["observed_at"])
    op.create_index(
        "ix_price_observations_route_date",
        "price_observations",
        ["origin_airport_id", "destination_id", "departure_date"],
    )
