...
- Владелец может уже сейчас подтвердить `hoptrip.pl` в Google Search Console через DNS TXT.
  TXT получена от владельца 2026-09-25; при проверке через 1.1.1.1 запись Google ещё
  отсутствует. Ожидаются добавление TXT в OVH и подтверждение в Search Console.
