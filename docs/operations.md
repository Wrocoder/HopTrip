...
## Удаление имени и адреса — 2026-09-25

Владелец запросил удаление персональных данных с сайта. Действующий
`/etc/hoptrip/operator.json` теперь содержит только email; поля имени, адреса и страны
удалены. `readOperator()` возвращает только email даже при старом формате файла.
На contact/privacy/terms выводится «Kontakt z HopTrip» с mailto, без адресного блока.
Регрессионный тест использует вымышленные legacy-поля и проверяет отсутствие этих
значений в полном HTML (включая серверные данные), на desktop/mobile.
Noindex юридических страниц и открытая индексация статей сохранены.
Описание первоначального запуска ниже является исторической записью.

## Публичный запуск индексации — 2026-09-25

- Contact/privacy/terms используют `legal-content.ts`; данные оператора только из
  `/etc/hoptrip/operator.json` (root:root 600), mount в web `/run/hoptrip/operator.json:ro`.
  Файл содержит строковые поля name/address/country/email; реальные значения не писать
  в репозиторий, образ, тесты и диагностические выводы. `connection()` исключает
  чтение данных при сборке. При отсутствии/ошибке файла показывается сообщение
  недоступности контактов; перед релизом обязательно проверить видимость блока оператора.
- После изменения конфигурации сохранять права 600 и пересоздать web, если файл заменён
  атомарно: bind mount может продолжать видеть старый inode. По той же причине после
  замены Caddyfile через архив нужен `up -d --no-deps --force-recreate caddy`.
- Caddyfile.domain открывает публичную индексацию, сохраняет admin 404, legal/technical
  noindex и www/sslip 308. Caddyfile.staging остаётся закрытым шаблоном для staging.
- Юридические страницы доступны, но исключены из sitemap/превью/JSON-LD. Drive не
  загружается на них; footer использует полную навигацию, чтобы выгрузить старый скрипт.
- Проверки: lint/typecheck, local и ARM64 production build, 84 desktop/mobile E2E.
  После финального изменения share-route ещё 2 целевых legal E2E прошли.
  Production SEO audit: 37 страниц, failures=[] (после пересоздания Caddy).
  На live проверены видимость контактов без вывода персональных данных, root 600,
  редиректы, admin 404 и все зелёные проверки monitor. Мобильная проверка с подменой
  Drive подтвердила загрузку после согласия на главной и выгрузку при переходе к контактам.
- Перед изменением сохранены image `hoptrip-web:before-indexing` и архив
  `/tmp/hoptrip-before-indexing.tgz` (исходники и конфиги, без файла оператора).
  Это временные артефакты отката, не offsite backup.
