...
**Обновление 2026-09-25:** публичные страницы завершены в
`apps/web/src/lib/legal-content.ts`, сведения оператора подставляются из серверного
файла вне Git через `operator.ts`. Текст ниже сохранён как исторический рабочий
черновик, а не актуальная публичная политика. Юридические страницы остаются noindex;
Drive на них отключён. Region VM проверен через OCI metadata: eu-frankfurt-1.
Почта: OVHcloud Zimbra, уведомления Telegram содержат только счётчики.
Публикация текста не является проверкой договоров в аккаунтах провайдеров:
владелец хранит применимые условия/DPA и обрабатывает обращения и права пользователей.
